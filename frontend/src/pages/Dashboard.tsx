...
export default function Dashboard() {
  ...
  if (status.error) return <ErrorState message="Erreur chargement statut système" />
  ...
  const topPicks = ideasData.filter(i => (i.score ?? 0) >= TOP_PICK_SCORE_THRESHOLD)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-0.5">Synthèse du cockpit d'analyse</p>
        </div>
      ...
      <div className="grid grid-cols-5 gap-4">
        ...
        <div className="card">
          ...
          <div className="stat-value">
            {latestT10Y2Y ? (
              <span
                className={
                  latestT10Y2Y.value < 0 ? 'text-red-400' : 'text-emerald-400'
                }
              >
          ...
          <div className="stat-label">
            10Y-2Y Spread{' '}
            {latestT10Y2Y?.value && latestT10Y2Y.value < 0 ? '(Inversée)' : ''}
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="stat-value">
            <span
              className={
                sectorsAbove > sectorsTotal / 2 ? 'text-emerald-400' : 'text-amber-400'
              }
            >
        ...
        <div className="card">
          <div className="card-header">Alertes Envoyées</div>
        ...
        <div className="card">
          ...
          <div className="stat-label">
            Idées score {'>='} {TOP_PICK_SCORE_THRESHOLD}
          </div>
        </div>
      </div>
          ...
          <div className="space-y-1.5">
            {sectorData.slice(0, 6).map(s => (
              <div key={s.symbol} className="flex items-center justify-between py-1">
                <div className="flex items-center gap-2">
                  <StatusBadge status={s.above_sma200} trueLabel="↑" falseLabel="↓" />
        ...
        <div className="card">
          ...
          {topPicks.length === 0 ? (
            <p className="text-xs text-zinc-500 py-4 text-center">
              Aucune idée au-dessus du seuil
            </p>
          ) : (
              ...
              {topPicks.slice(0, 5).map((idea, i) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {idea.rank != null && (
                      <span className="text-[10px] text-zinc-500">#{idea.rank}</span>
                    )}
                  </div>
                </div>
              ))}
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="text-zinc-500 border-b border-zinc-800">
                  ...
                  <th className="text-left py-2 font-medium">Déclencheur</th>
                ...
                {alertData.map(a => (
                  <tr key={a.id} className="table-row">
                    <td className="py-2">
                      <span
                        className={
                          a.alert_type === 'equity' ? 'badge-blue' : 'badge-yellow'
                        }
                      >
                        {a.alert_type}
                      </span>
